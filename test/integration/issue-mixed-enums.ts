...
import json from './fixtures/issue-mixed-enums.json'
import { sync as rmrf } from 'rimraf'

const outputDir = path.join(__dirname, 'output', 'issue-mixed-enums')
const generatedSDKDir = path.join(outputDir, 'generated')

test('renders type correct SDK for issue-mixed-enums', async (t) => {
  rmrf(outputDir)
